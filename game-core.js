...
function petTierInstinctText(p) {
  ...
  return `${unlocked.join("｜")}${next ? `｜下一特性：${next[0]}阶【${next[1].name}】` : "｜10阶后仍可无限融合；每升1阶继续增加12%基础全域倍率，但不再新增本能机制。"}`;
}
...
function identityMaxRarityForMap(m = map()) {
  const difficulties = window.ALPHA_041_DIFFICULTIES;
  if (Array.isArray(difficulties)) {
    const index = Math.max(
      0,
      difficulties.findIndex((x) => x.id === state.worldDifficulty),
    );
...
function rollIdentityRarity(maxR) {
  const w = maxR >= 5 ? [18, 16, 15, 15, 16, 20] : [42, 27, 16, 9, 4.5, 1.5],
    pool = [];
...
const MYTHIC_IDENTITY_BASE_CHANCE = 0.06;
const MYTHIC_IDENTITY_STREAK_STEP = 0.01;
const MYTHIC_IDENTITY_SOFT_CAP = 0.2;
const MYTHIC_IDENTITY_SOFT_CAP_STREAK = 14;
function lockedMythicIdentities() {
  ensureIdentityState();
  return [
    ...Object.keys(RACES)
      .filter((id) => RACES[id].rarity === 5 && !state.unlockedRaces.includes(id))
      .map((id) => ({ kind: "race", id })),
    ...Object.keys(STYLES)
      .filter((id) => STYLES[id].rarity === 5 && !state.unlockedClasses.includes(id))
      .map((id) => ({ kind: "class", id })),
  ];
}
function mythicIdentityPityText() {
  const maxR = identityMaxRarityForMap(),
    missing = lockedMythicIdentities().length,
    streak = Math.max(0, Math.round(Number(state.mythicIdentityPity || 0)));
  if (!missing) return "神话身份已全部解锁";
  if (maxR < 5) return "神话身份：折磨 VI 开始掉落";
  return `神话身份：下一场独立解锁概率 ${(mythicIdentityUnlockChance() * 100).toFixed(0)}%（连续未出 ${streak} 场，无硬保底）`;
}
function mythicIdentityUnlockChance() {
  const streak = Math.max(0, Number(state.mythicIdentityPity || 0));
  return Math.min(
    MYTHIC_IDENTITY_SOFT_CAP,
    MYTHIC_IDENTITY_BASE_CHANCE + streak * MYTHIC_IDENTITY_STREAK_STEP,
  );
}
function tryDropIdentity(e, m) {
  ...
  const maxR = identityMaxRarityForMap(m),
    missingMythics = lockedMythicIdentities(),
    mythicEligible = maxR >= 5 && missingMythics.length > 0,
    directMythic =
      mythicEligible && Math.random() < mythicIdentityUnlockChance();
  if (!directMythic && Math.random() >= identityDropChance(e.bossLootMult || 1)) {
    state.identityPity = (state.identityPity || 0) + 1;
    if (mythicEligible)
      state.mythicIdentityPity = Math.min(
        MYTHIC_IDENTITY_SOFT_CAP_STREAK,
        state.mythicIdentityPity + 1,
      );
    return false;
  }
  const rarity = directMythic ? 5 : rollIdentityRarity(maxR),
    racePool = Object.keys(RACES).filter((id) => RACES[id].rarity === rarity),
    classPool = Object.keys(STYLES).filter(
      (id) => STYLES[id].rarity === rarity,
    );
  const forced = directMythic
    ? missingMythics[rnd(0, missingMythics.length - 1)]
    : null;
  let kind = forced?.kind || (Math.random() < 0.5 ? "race" : "class"),
  ...
  const locked = pool.filter((id) => !unlocked.includes(id)),
    chosenPool = forced
      ? [forced.id]
      : locked.length && Math.random() < 0.85
        ? locked
        : pool,
  ...
  if (!unlocked.includes(id)) {
    ...
    if (def.rarity === 5) state.mythicIdentityPity = 0;
    else if (mythicEligible)
      state.mythicIdentityPity = Math.min(
        MYTHIC_IDENTITY_SOFT_CAP_STREAK,
        state.mythicIdentityPity + 1,
      );
...
const PET_MUTATION_BASE_CHANCE = 0.05;
const PET_MUTATION_STREAK_STEP = 0.01;
const PET_MUTATION_SOFT_CAP = 0.2;
const PET_MUTATION_SOFT_CAP_STREAK = 15;
function petMutationChance(mapDef = map(), lootMult = 1, streakEligible = false) {
  const mult = Math.max(1, lootMult),
    luck = stats().luck,
    d = dangerDropProfile(mapDef.id),
    base = Math.min(0.009, 0.0025 + luck * 0.000012),
    natural = Math.min(0.042, 0.014 * mult, base * d.mutation * mult),
    streak = streakEligible
      ? Math.max(0, Number(state.petMutationPity || 0))
      : 0;
  return Math.min(
    PET_MUTATION_SOFT_CAP,
    (streakEligible ? Math.max(PET_MUTATION_BASE_CHANCE, natural) : natural) +
      streak * PET_MUTATION_STREAK_STEP,
  );
}
function rollPetMutation(mapDef = map(), lootMult = 1, pityEligible = false) {
  const streak = Math.max(0, Number(state.petMutationPity || 0)),
    mutant = Math.random() < petMutationChance(mapDef, lootMult, pityEligible);
  if (pityEligible)
    state.petMutationPity = mutant
      ? 0
      : Math.min(PET_MUTATION_SOFT_CAP_STREAK, streak + 1);
  return mutant;
}
function petMutationPityText() {
  const streak = Math.max(0, Math.round(Number(state.petMutationPity || 0)));
  return `变异X：下一只Boss宠物概率 ${(petMutationChance(map(), 1, true) * 100).toFixed(0)}%（连续未出 ${streak} 只，无硬保底）`;
}
...
/* ===== core-09.js ===== */
function createPet(name, type, mapIndex, lootMult = 1, mutationPityEligible = false) {
  const m = MAPS[mapIndex] || map(),
    tier = rollPetTier(m),
    mutant = rollPetMutation(m, lootMult, mutationPityEligible),
    p = {
      id: "p" + state.nextId++,
      name,
...
function renderDefeatReport() {
  const report = state.lastDefeatReport,
    summary = report
      ? `<span><b>战败诊断</b><small>${report.primary} · ${report.enemy}剩余${report.enemyHpPct}%</small></span><i>${defeatReportOpen ? "收起" : "查看"}</i>`
      : `<span><b>战败诊断</b><small>战败后生成针对性建议</small></span><i>暂无</i>`;
  return `<div class="defeat-report-slot"><button class="defeat-report-trigger" type="button" ${report ? `onclick="toggleDefeatReport()" aria-expanded="${defeatReportOpen}"` : "disabled"}>${summary}</button></div>${report && defeatReportOpen ? `<div class="defeat-report-overlay" onclick="if(event.target===this)toggleDefeatReport(false)"><section class="defeat-report-sheet" role="dialog" aria-modal="true" aria-labelledby="defeat-report-title"><div class="defeat-report-head"><div><b id="defeat-report-title">上次战败诊断：${report.primary}</b><span>${report.enemy} · 剩余${report.enemyHpPct}% · ${report.rounds}回合</span></div><button type="button" class="defeat-report-close" onclick="toggleDefeatReport(false)" aria-label="关闭战败诊断">关闭</button></div><div class="defeat-report-body">${report.reasons.map((x) => `<div><b>${x.name}</b>：${x.advice}</div>`).join("")}</div></section></div>` : ""}`;
}
function loseBattle() {
  state.totalLosses++;
  const e = state.enemy,
...
function renderStart() {
  ...
  app.innerHTML = `<div class="start"><h1>无尽战域：Alpha 0.45.5</h1><p class="subtitle">五分钟做构筑 · 全天自动刷宝</p><label>角色名称 <input id="hero-name" value="旅者" style="margin-left:8px;background:#12100c;color:#fff;border:1px solid #51442f;padding:7px"></label><h2>选择普通种族</h2><div class="choice-grid">${STARTER_RACES.map(
    (id) => {
      const r = RACES[id];
...
function renderPets() {
  const f = state.petFilter || {
    ...
    maxFilterTier = Math.max(
      PET_TIER_MAX_UI,
      Number(f.minTier || 1),
      ...state.pets.map((p) => Math.max(1, Number(p.tier || 1))),
    );
