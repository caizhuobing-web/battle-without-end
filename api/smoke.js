...
module.exports=(req,res)=>{
 try{
  ...
  const chunks=files.map(f=>fs.readFileSync(path.join(process.cwd(),f),'utf8'));
  ...
  res.status(missing.length?500:200).json({ok:missing.length===0,version:'0.34.0',files:files.length,bytes:code.length,missing});
